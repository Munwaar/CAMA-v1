...
function Mentors() {
  ...
  const mentor = { name: "Arun K.", role: "Java Backend Developer", match: 92, language: "Tamil + English", style: "Project-focused", availability: "Mon–Sat, 9:00 AM – 8:00 PM" };
  const nextMentor = { name: "Meera S.", role: "Data Engineer", match: 84 };
  ...
  return <div className="page-stack vibe-view">
    ...
    <section className="vibe-steps">
      <article className={`vibe-step ${step === 1 ? "selected" : ""}`} onClick={() => goStep(1)}><div className="step-number">1</div><h3>Enter Vibe Mentor</h3><p>Click on Vibe Mentor from the sidebar.<br />You’ll land on the mentor discovery page.</p><div className="mini-discovery"><div className="mini-sidebar"><CamaMark size={15} /><b>CAMA</b><span>⌂ Home</span><span>◎ Skill DNA</span><span>◉ Gap Radar</span><span className="active">◉ Vibe Mentor</span></div><div className="mini-panel"><div className="mini-panel-title">Vibe Mentor <small>Find your perfect mentor match</small></div><div className="mini-search">⌕ Search mentors...</div><div className="mini-mentor"><div className="avatar-photo arun">AK</div><div><b>Arun K.</b><small>Java Backend Developer</small><em>◉ 92% Vibe Match</em></div><Heart size={13} /></div><div className="mini-tags"><span>Project-focused</span><span>Tamil + English</span></div><small>Experience: 3+ years<br />Next slot: Today, 4:00 PM</small><div><button onClick={(e) => { e.stopPropagation(); goStep(3); }}>Preview 10 min</button><button className="dark-mini" onClick={(e) => { e.stopPropagation(); goStep(2); }}>View Profile</button></div></div></div></article>
