...
function Mentors() {
  ...
  const mentors = [{ name: "Arun K.", focus: "Java backend", vibe: "Project-focused", language: "Tamil + English", capacity: "12 / 15 students", match: "92% vibe match" }, { name: "Meera S.", focus: "Data engineering", vibe: "Slow + detailed", language: "English + Hindi", capacity: "8 / 12 students", match: "84% vibe match" }];
  return <div className="page-stack"><SectionHeading eyebrow="MENTORS / OPTIONAL SUPPORT" title="Choose guidance that matches your way of learning." description="Self-paced is always a first-class route. Mentors are an optional layer for doubt clearing, project guidance, and career preparation." action={<Pill tone="blue">VIBE MATCHING ON</Pill>} /><section className="mentor-preferences"><div><div className="eyebrow">YOUR LEARNING VIBE</div><h3>Project-based · Tamil + English</h3><p>We use your pace, language, and preference to make the match—not a star rating.</p></div><div className="vibe-pills"><Pill tone="orange">Project-focused</Pill><Pill tone="blue">Tamil + English</Pill><Pill tone="moss">Beginner-friendly</Pill></div></section><section className="mentor-grid">{mentors.map((mentor) => <article className="mentor-card atlas-card" key={mentor.name}><div className="mentor-avatar">{mentor.name.slice(0, 1).toUpperCase()}</div><div className="mentor-head"><div><h3>{mentor.name}</h3><span>{mentor.focus}</span></div><Pill tone="orange">{mentor.match}</Pill></div><div className="mentor-details"><span><Sparkles size={14} /> {mentor.vibe}</span><span><BookOpen size={14} /> {mentor.language}</span><span><UsersIcon /> {mentor.capacity}</span></div><div className="mentor-actions"><button className="secondary-button" onClick={() => setPreview(mentor.name)}>Preview</button><button className="primary-button small" onClick={() => toast(`${mentor.name} is ready to guide your next project.`)}>Guide me <ArrowRight size={14} /></button></div></article>)}</section>{preview && <div className="preview-strip"><div><div className="eyebrow">10-MINUTE INTRO PREVIEW / {preview.toUpperCase()}</div><h3>Test the teaching style before you commit.</h3><p>Preview is limited to compatibility, a short concept, and an introduction—never unlimited free learning.</p></div><div className="preview-actions"><button className="secondary-button" onClick={() => setPreview(null)}>Skip</button><button className="primary-button" onClick={() => toast("Mentor guidance selected for this prototype session.")}>Guide me <ArrowRight size={15} /></button></div></div>}</div>;
}
